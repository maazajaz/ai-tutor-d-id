  ...
  const handleAskQuestion = async (question = userInput) => {
    ...
    try {
      ...
      if ((diagramType === 'image' || diagramType === 'dalle_image') && imageUrl) {
        ...
        // On mobile, don't draw on canvas - just save to database and show as card
        if (isMobile) {
          console.log('📱 Mobile view: Saving as card instead of canvas');
          ...
          if (whiteboardSessionId) {
            const { data: savedContent, error } = await saveWhiteboardContent(whiteboardSessionId, {
              ...
              canvasData: null, // No canvas data on mobile
              elements: [],
              positionY,
              height: 0, // No canvas height on mobile
              canvasWidth: 0,
              imageUrl,
        ...
        // Desktop: Draw on canvas as before
        ...
        img.onload = async () => {
          ...
          // Calculate max width for desktop (95% of canvas width)
          const maxImageWidth = canvas.width * 0.95;
          const maxImageHeight = blockHeight * 0.85;
